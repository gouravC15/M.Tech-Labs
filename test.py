...
if __name__ == "__main__":
    ...
    input_img = cv2.imread("/Users/gouravchirkhare/Desktop/Images/Test_image.png")
    ...
    i = plt.imshow(input_img)  # YlGn Set2 gist_earth ocean viridis
    ax.set_title('Test Labeled')
    # plt.colorbar(ticks=[0.1, 0.3, 0.5], orientation='horizontal')
    ax = fig.add_subplot(1, 2, 2)
    j = plt.imshow(segmented, cmap='viridis')
    ax.set_title('Segmented')
    plt.colorbar(label="Vegetation 0=High, 3=Low", orientation="vertical")
    '''
    ax.plot(1, color='lime', label='Dense Vegetation')
    ax.plot(1, color='darkblue', label='Spare Vegetation')
    ax.plot(1, color='yellow', label='Water')
    ax.plot(1, color='lightcoral', label='Land')

    fig2 = plt.figure()
    leg = ax.legend(loc='right', bbox_to_anchor=(0.8, -0.20), shadow=False, ncol=4)
'''
    plt.show()